...
from LaserSer import LaserSer
from SetupLaser import SetupLaser


def MainExpSLASER(imgPath,stimPath, clock, kb, mywin, pains, painsFiles, subjectNum, subjPath, sessionNum, thisBlock, RatScaleOr):
    ...
    for i in range (0,nTrials):
        ...
        if ABin[i,0] == 1:
            ...
            allPain['pPainNew'][i] = (allPain['pPain'][i])/2
        elif ABin[i,0] == 2:
        ...
        print('The trial ' + str(i) + ' took ' + str(durTrial[i]) + ' seconds.')
        
        #save results
        cwd = os.getcwd()
        if cwd != subjPath:
            os.chdir(subjPath)
        Responses = pd.DataFrame(ABin)
        Responses.to_csv('Responses' + subjectNum + '_' + str(sessionNum) + Condition + '.csv')
        allPain.to_csv('AllPain' + subjectNum + '_' + str(sessionNum) + Condition + '.csv')

            
    
    RiPE, PPE, EP2, pRelief, Cost, vPain, RiskPain, RiskPain2, EA, Outcome, ABin, EVChoice, EP2p= computeDecVars(ABin, Outcome, i, nTrials, allPain)    
    ...
    AllVariables = pd.concat([allPain, RiPE, PPE, EP2, pRelief, Cost, vPain, RiskPain, RiskPain2, EA, Outcome, ABin, EVChoice, EP2p, TP1,TP2, TP3, TP4, TP5, Rate, durTrial, startTrial], axis = 1);
